...
#' @param w         Manual choice of weights.
...
#'
#' reconst <- ReconstFD(Y0.obs, Y1.obs, T.set = c(1), pred.band = TRUE)
...
ReconstFD <- function (Y0.obs, Y1.obs = NULL, T.set = 1:dim(Y0.obs)[1],
  ...
  if(is.null(Y1.obs)) {
    ...
    for (t in T.set) {
      ...
      F.hat.t <- 1/sqrt(N) * Y.obs.w[O.t] %*% svd$v %*%
        diag(1/svd$d[1:r.hat[t.idx]], nrow = r.hat[t.idx], ncol = r.hat[t.idx])
...
PredBand <- function(Y0.c, Y1.c = NULL, O.set, p = 0.95, r.max = 20, ro) {
  ...
  for(t in 1:Tc) {
    Y0.obs <- Y0.c
    Y0.obs[t, -O.set] <- NA
    reconst <- ReconstFD(Y0.obs, Y1.c, T.set = c(t), method = "manual",
